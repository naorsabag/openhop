...
- Limit each audit attempt to two minutes so a stalled registry request is treated as transient.
...
4. Repeated hung requests time out, stop after three attempts, and fail.

Run shell syntax checks, the helper test, repository formatting checks, and the GitHub Actions workflow.

## Scope

Only `.github/workflows/ci.yml`, the retry helper, its test, and its design and implementation documentation are changed. Dependency versions and application behavior are unaffected.
