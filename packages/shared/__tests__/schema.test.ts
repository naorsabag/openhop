...
const fullFlow = {
  ...
  flow: {
    nodes: [
      ...
      { id: "cache", label: "Cache", type: "cache" },
      {
        ...
        flow: {
          ...
          steps: [
            { from: "inner-a", to: "inner-b", data: "inner data" },
          ],
        },
      },
    ],
    steps: [
      ...
      {
        parallel: [
          { from: "svc", to: "db", data: { label: "Query", fields: [{ name: "id", type: "int" }, { name: "status", changed: true }, { name: "email", added: true }] } },
...
describe("Schema validation", () => {
  ...
  it("rejects missing required fields", () => {
    ...
    expect(result.errors.some((e) => e.path.includes("title") || e.message.includes("title") || e.message.includes("Required") || e.message.includes("too_small"))).toBe(true);
  });
